...
class LicenseKeyGeneratorTest {
    ...
    @Property
    void should_return_valid_license_key_random_valid_arguments(@ForAll("validArguments")
                                                                Tuple.Tuple3<Integer,
                                                                        Integer,
                                                                        Integer> validArguments) {
    ...
    @Property
    void should_return_invalid_license_key_random_invalid_arguments(@ForAll("inValidArguments")
                                                                Tuple.Tuple3<Integer,
                                                                        Integer,
                                                                        Integer> validArguments) {
        int numberOfSection = validArguments.get1();
    ...
    Arbitrary<Tuple.Tuple3<Integer, Integer, Integer>> inValidArguments() {
        Arbitrary<Integer> invalidNumberOfSection = Arbitraries.oneOf(
                Arbitraries.integers().between(Integer.MIN_VALUE, 2),
                Arbitraries.integers().between(7, Integer.MAX_VALUE)
        );

        Arbitrary<Integer> invalidNumberOfCharacter = Arbitraries.oneOf(
                Arbitraries.integers().between(Integer.MIN_VALUE, 2),
                Arbitraries.integers().between(7, Integer.MAX_VALUE)
        );

        Arbitrary<Integer> invalidNumberOfDigit = Arbitraries.oneOf(
                Arbitraries.integers().between(Integer.MIN_VALUE, 2),
                Arbitraries.integers().between(7, Integer.MAX_VALUE)
        );

        return Combinators.combine(
                invalidNumberOfSection,
                invalidNumberOfCharacter,
                invalidNumberOfDigit
        ).as(Tuple::of);
    }
