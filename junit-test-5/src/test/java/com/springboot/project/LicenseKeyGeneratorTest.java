...
class LicenseKeyGeneratorTest {
    ...
    @Test
    @DisplayName(
            "should return valid licenseKey with 3 sections and length is 10 with 5 character and 5 digit")
    ...
    @ParameterizedTest(
            name =
                    "[{index}] sections: {0} - sectionLength: {1} - numOfCharacter: {2} - numOfDigit: {3}")
    @CsvSource({"3,10,5,5", "4,6,3,3"})
    @DisplayName("should return valid licenseKey with input csv source")
    void should_return_valid_license_key_with_parameterized_csv_source(
            int numberOfSection, int sectionLength, int numOfCharacter, int numOfDigit) {
        String sectionPattern = this.generateSectionPattern(numberOfSection);
    ...
    void should_return_valid_license_key_with_parameterized_csv_source_with_argument_accessor(
    ...
    void should_return_valid_license_key_with_parameterized_csv_file_source(
            int numberOfSection, int sectionLength, int numOfCharacter, int numOfDigit) {
        String sectionPattern = this.generateSectionPattern(numberOfSection);
        LicenseKeyGenerator generator =
                new LicenseKeyGenerator(numberOfSection, sectionLength, numOfCharacter, numOfDigit);
    ...
    void should_return_valid_license_key_with_parameterized_method_source(
            int numberOfSection, int sectionLength, int numOfCharacter, int numOfDigit) {
        String sectionPattern = this.generateSectionPattern(numberOfSection);
        LicenseKeyGenerator generator =
                new LicenseKeyGenerator(numberOfSection, sectionLength, numOfCharacter, numOfDigit);
    ...
    @MethodSource
    @DisplayName(
            "should return valid licenseKey with input using method source contain same test method name")
    void should_return_valid_license_key_with_parameterized_method_source_same_test_method_name(
            int numberOfSection, int sectionLength, int numOfCharacter, int numOfDigit) {
        String sectionPattern = this.generateSectionPattern(numberOfSection);
        LicenseKeyGenerator generator =
                new LicenseKeyGenerator(numberOfSection, sectionLength, numOfCharacter, numOfDigit);
    ...
    private static Stream<Arguments>
            should_return_valid_license_key_with_parameterized_method_source_same_test_method_name() {
        return Stream.of(Arguments.of(3, 10, 5, 5), Arguments.of(4, 6, 3, 3));
    }
    ...
    @DisplayName(
            "should return valid licenseKey with input using method source with sharing methods")
    void should_return_valid_license_key_with_parameterized_method_source_with_sharing_methods(
            int numberOfSection, int sectionLength, int numOfCharacter, int numOfDigit) {
        String sectionPattern = this.generateSectionPattern(numberOfSection);
        LicenseKeyGenerator generator =
                new LicenseKeyGenerator(numberOfSection, sectionLength, numOfCharacter, numOfDigit);
    ...
    @RepeatedTest(30)
    @DisplayName(
            "should return valid licenseKey with input using method source and random arguments")
    void should_return_valid_license_key_with_repeated_test_and_random_arguments(
