...
public class CustomDateTimeValidator implements FormatValidator {
    ...
    static {
        final DateTimeParser secFracsParser =
                new DateTimeFormatterBuilder()
                        .appendLiteral('.')
                        .appendFractionOfSecond(1, 12)
                        .toParser();

        final DateTimeParser timeZoneOffset =
                new DateTimeFormatterBuilder().appendTimeZoneOffset("Z", false, 2, 2).toParser();
    ...
    public Optional<String> validate(String dateTime) {
        try {
            ...
            return Optional.of(
                    String.format(
                            "[%s] is not a valid %s. Expected %s",
                            dateTime, formatName(), FORMATS));
        }
    }
