...
public class DroolsService {
    ...
    public String createDrlFileFromJsonNode(JsonNode jsonNode) {
        ...
        for (JsonNode rule : rules) {
            ...
            JsonNode conflicts = rule.get("conflicts");
            if (conflicts != null && conflicts.isArray()) {
                for (JsonNode conflict : conflicts) {
                    RuleDescr conflictRule = new RuleDescr();
                    conflictRule.setName(conflict.get("name").asText());
                    PatternDescr conflictPattern = new PatternDescr();
                    conflictPattern.setIdentifier(whenNodes.get(0).get("identifier").asText());
                    conflictPattern.setObjectType(whenNodes.get(0).get("type").asText());
                    for (JsonNode conflictCondition : conflict.get("conditions")) {
                        ExprConstraintDescr conflictExpr = new ExprConstraintDescr();
                        conflictExpr.setText(conflictCondition.asText());
                        conflictPattern.addConstraint(conflictExpr);
                    }
                    AndDescr conflictLhs = new AndDescr();
                    conflictLhs.addDescr(conflictPattern);
                    conflictRule.setLhs(conflictLhs);
                    StringBuilder conflictConsequence = new StringBuilder();
                    for (JsonNode action : conflict.get("actions")) {
                        conflictConsequence.append(action.asText()).append(";");
                    }
                    conflictRule.setConsequence(conflictConsequence.toString());
                    packageDescr.addRule(conflictRule);
                }
            }
        }
